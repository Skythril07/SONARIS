...
const stages = ["UPLOAD", "DECODE", "CORRECT", "ENHANCE", "DETECT", "LOCATE", "VERIFY", "REPORT", "DONE"];
const API = ""; // relative — Vite proxies /api in dev; same-origin when served from dist
const navItems = [
...
// ---- backend integration helpers (Phase 3) --------------------------------------------------
const CLASS_LABEL = { fishing_gear: "Fishing Gear", pipe_cable: "Pipe / Cable", wreck_large_object: "Wreck", man_made_anomaly: "Anomaly", stub: "Contact" };
const CLASS_SHORT = { fishing_gear: "NET", pipe_cable: "PIPE", wreck_large_object: "WRK", man_made_anomaly: "ANOM", stub: "CON" };
const round1 = (v) => Math.round((v || 0) * 10) / 10;
const today = () => new Date().toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" });

function deriveVerdict(d) {
  const rs = (d.review_status || "").toUpperCase();
  if (rs === "CONFIRMED" || rs === "REJECTED") return rs;
  if (rs === "NEEDS_REVIEW") return "REVIEW";
  if (d.metric_plausible === false) return "REVIEW";
  if ((d.confidence || 0) >= 0.85) return "HIGH";
  return "REVIEW";
}
function noteFor(d) {
  const ev = d.evidence || {};
  const bits = [];
  if (ev.shadow) bits.push(`shadow drop ${Math.round((ev.shadow.shadow_drop_frac || 0) * 100)}%`);
  if (d.persistence_count > 1) bits.push(`seen across ${d.persistence_count} tiles`);
  bits.push(`±${round1(d.position_uncertainty_m)} m position error`);
  return bits.join(" · ") || "Georeferenced acoustic contact.";
}
function mapDetections(dets, track) {
  const pts = track?.features?.[0]?.geometry?.coordinates || [];
  let minLon = Infinity, maxLon = -Infinity, minLat = Infinity, maxLat = -Infinity;
  const seen = [...pts.map(([lon, lat]) => ({ lon, lat })), ...dets.map((d) => ({ lon: d.lon, lat: d.lat }))];
  for (const p of seen) { minLon = Math.min(minLon, p.lon); maxLon = Math.max(maxLon, p.lon); minLat = Math.min(minLat, p.lat); maxLat = Math.max(maxLat, p.lat); }
  const spanLon = maxLon - minLon || 1, spanLat = maxLat - minLat || 1, pad = 10;
  return dets.map((d) => ({
    id: d.detection_id,
    type: CLASS_LABEL[d.cls] || d.cls,
    short: CLASS_SHORT[d.cls] || (d.cls || "?").slice(0, 3).toUpperCase(),
    confidence: Math.round((d.confidence || 0) * 100),
    lat: d.lat, lon: d.lon,
    size: round1(Math.max(d.length_m || 0, d.width_m || 0)),
    persistence: d.persistence_count || 1,
    verdict: deriveVerdict(d),
    x: pad + ((d.lon - minLon) / spanLon) * (100 - 2 * pad),
    y: pad + (1 - (d.lat - minLat) / spanLat) * (100 - 2 * pad),
    shadow: d.shadow_consistent === true ? "PASS" : d.shadow_consistent === false ? "FAIL" : "REVIEW",
    uncertainty: round1(d.position_uncertainty_m),
    note: noteFor(d),
    survey: d.line_id || "",
  }));
}
...
function App() {
  ...
  async function runJob(file) {
    ...
    setProcessing(true); setStageIndex(0); showNotice("Uploading survey…");
    setFileName(file.name); navigate("workspace");
    try {
      const fd = new FormData(); fd.append("file", file);
      const res = await fetch(`${API}/api/jobs`, { method: "POST", body: fd });
      if (!res.ok) throw new Error("upload failed");
      const job = await res.json(); setJobId(job.id);
      const sid = job.id.slice(0, 6).toUpperCase();
      setSurveys((s) => [{ id: sid, name: file.name.replace(/\.[^.]+$/, ""), file: file.name, date: today(), status: "PROCESSING", detections: 0 }, ...s]);
      const es = new EventSource(`${API}/api/jobs/${job.id}/events`);
      es.addEventListener("stage", (e) => {
        const idx = stages.indexOf(JSON.parse(e.data).stage);
        if (idx >= 0) setStageIndex(idx);
      });
      es.addEventListener("complete", async (e) => {
        es.close();
        const info = JSON.parse(e.data);
        if (info.status === "error") { setProcessing(false); showNotice("Analysis failed — see server log"); return; }
        setStageIndex(stages.length - 1);
        const [dets, track] = await Promise.all([
          fetch(`${API}/api/jobs/${job.id}/detections`).then((r) => r.json()),
          fetch(`${API}/api/jobs/${job.id}/track`).then((r) => (r.ok ? r.json() : null)).catch(() => null),
        ]);
        const mapped = mapDetections(dets, track);
        setDetections(mapped);
        if (mapped[0]) setSelectedId(mapped[0].id);
        setSurveys((s) => s.map((sv) => (sv.id === sid ? { ...sv, status: "ANALYZED", detections: mapped.length } : sv)));
        setProcessing(false);
        showNotice(`Analysis complete · ${mapped.length} detection${mapped.length === 1 ? "" : "s"}`);
      });
      es.onerror = () => { es.close(); setProcessing(false); showNotice("Lost connection to analysis stream"); };
    } catch {
      setProcessing(false); showNotice("Upload failed — is the backend running? (sonaris serve)");
    }
  }
  ...
  async function updateVerdict(value) {
    setDetections((prev) => prev.map((d) => d.id === selectedId ? { ...d, verdict: value } : d));
    showNotice(`Detection #${selectedId} marked ${value}`);
    if (jobId) {
      try { await fetch(`${API}/api/detections/${selectedId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ review_status: value }) }); } catch { /* keep local state */ }
    }
  }
  function exportReport(format) {
    if (jobId) { window.open(`${API}/api/jobs/${jobId}/report/${format}`, "_blank"); showNotice(`${format.toUpperCase()} exported`); return; }
